...
draw_char (int x, int align, wchar_t ch)
{
    ...
    switch (align) {
        case ALIGN_C:
            ...
            break;
        case ALIGN_R:
            x = ((bar_width + BAR_OFFSET < screen_width) ? x + BAR_UNDERLINE_HEIGHT : x);
            xcb_copy_area (c, canvas, canvas, draw_gc, bar_width - x, 0, 
                    bar_width - x - ch_width, 0, x, BAR_HEIGHT);
            x = ((bar_width + BAR_OFFSET < screen_width) ? bar_width - ch_width - BAR_UNDERLINE_HEIGHT : bar_width - ch_width); 
            break;
        case ALIGN_L:
            x = ( (BAR_OFFSET) ? x + BAR_UNDERLINE_HEIGHT : x ); 
            break;
    }
    ...
    xcb_image_text_16 (c, 1, canvas, draw_gc, x, BAR_HEIGHT / 2 + sel_font->avg_height / 2 - sel_font->descent + (BAR_UNDERLINE_HEIGHT / 2), (xcb_char2b_t *)&ch);
...
void
draw_base (void) {
    /* draw the background and the default underline if there is one */
    xcb_fill_rect (clear_gc, 0, 0, bar_width, BAR_HEIGHT);
    if (BAR_UNDERLINE_DEFAULT) {
        xcb_fill_rect (underl_gc, 0, BAR_BOTTOM*(BAR_HEIGHT-BAR_UNDERLINE_HEIGHT), bar_width, BAR_UNDERLINE_HEIGHT);
        if (BAR_OFFSET)
            xcb_fill_rect (underl_gc, 0, 0, BAR_UNDERLINE_HEIGHT, BAR_HEIGHT);
        if ((bar_width+BAR_OFFSET) < screen_width)
            xcb_fill_rect (underl_gc, bar_width-BAR_UNDERLINE_HEIGHT, 0, BAR_UNDERLINE_HEIGHT, BAR_HEIGHT);
    }
}
        ...
        if (*p == '\\' && p++ && *p != '\\' && strchr ("fbulcr", *p)) {
                switch (*p++) {
                    ...
                    case 'b': 
                        ...
                        break;
                    case 'u': 
                        if (!isdigit (*p)) *p = BAR_UNDERLINE_DEFAULT; 
                        xcb_set_ud ((*p++)-'0');
                        break;
...
init (void)
{
    ...
    screen_width = scr->width_in_pixels;
    bar_width = (BAR_WIDTH < 0) ? (screen_width - BAR_OFFSET) : BAR_WIDTH;
    ...
    xcb_create_window (c, XCB_COPY_FROM_PARENT, win, root, BAR_OFFSET, y, bar_width,
            ...
            XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK, (const uint32_t []){ palette[0], XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_BUTTON_PRESS });
    ...
    xcb_create_gc (c, underl_gc, root, XCB_GC_FOREGROUND, (const uint32_t []){ palette[BAR_UNDERLINE_DEFAULT] });
...
main (int argc, char **argv)
{
    ...
    xcb_button_press_event_t *bp_ev;
                ...
                while ((ev = xcb_poll_for_event (c))) {
                    switch (ev->response_type & 0x7F) {
                        ...
                        case XCB_BUTTON_PRESS:
                            bp_ev = (xcb_button_press_event_t *)ev;
                            system("urxvt");
                            break;
                    }
